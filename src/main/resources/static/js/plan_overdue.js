$(function() {
    ...
    $("#input_search_submit").click(function() {
    	window.location.href = "/plan/findOverdue?search=" + $("#input_search").val();
    });
    ...
    $("#input_checkbox_all").click(function() {
        ...
        if ($("#input_checkbox_all").prop("checked")) {
            for (var counter = 0; counter < $(".plan-id").length; counter++) {
                $("#input_remove_all").val($("#input_remove_all").val() + $(".plan-id").eq(counter).val() + ", ");
            }
        } 
    });
    ...
    $(".input-checkbox").on("click", function() {
        if ($(this).prop("checked")) {
            $("#input_remove_all").val($("#input_remove_all").val() + $(this).parent().parent().next().next().next().next().next().next().next().next().next().find(".plan-id").val() + ", ");
        } else {
            $("#input_remove_all").val($("#input_remove_all").val().replace($(this).parent().parent().next().next().next().next().next().next().next().next().next().find(".plan-id").val() + ", ", ""));
        }
    });
        ...
        $("#input_remove_submit").click(function() {
            if (id > 0) {
            	$.post("/plan/removeByIds", {
                    ...
                    window.location.href = "/plan/findOverdue?search=" + $("#input_search").val() + "&currentPage=" + $("#a_current_page").text();
                });
            }
        });
