$(function() {
    ...
    $(".input-checkbox").on("click", function() {
        if ($(this).prop("checked")) {
            $("#input_remove_all").val($("#input_remove_all").val() + $(this).parent().parent().next().next().next().next().next().next().next().next().next().find(".activity-id").val() + ", ");
        } else {
            $("#input_remove_all").val($("#input_remove_all").val().replace($(this).parent().parent().next().next().next().next().next().next().next().next().next().find(".activity-id").val() + ", ", ""));
        }
    });
    
    $("input[value = '重办']").on("click", function() {
        ...
        $("#input_create_name").val($(this).parent().prev().prev().prev().prev().prev().prev().prev().text());
        $("#input_create_date").val(currentDate);
        $("#input_create_place").val($(this).parent().prev().prev().prev().prev().prev().text());
    ...
    $("input[value = '删除']").on("click", function() {
        $("#span_remove_name").text($(this).parent().prev().prev().prev().prev().prev().prev().prev().text());
        $("#span_remove_date").text($(this).parent().prev().prev().prev().prev().prev().prev().text());
        $("#span_remove_place").text($(this).parent().prev().prev().prev().prev().prev().text());
        var id = $(this).next().val();
        
        $("#input_remove_submit").click(function() {
            if (id > 0) {
            	$.post("/activity/removeByIds", {
                    ids: id + ", "
                }, function(data, status) {
                    if ("success" == data) {
                        alert("删除成功！")
                    ...
                    window.location.href = "/activity/findOverdue?search=" + $("#input_search").val() + "&currentPage=" + $("#a_current_page").text();
                });
            }
        });
    ...
    $("#input_remove_all_submit").click(function() {
        if (!$("#input_remove_all").val()) {
        	alert("请选择至少一条记录！");
        	window.location.reload();
        }
        
        $.post("/activity/removeByIds", {
            ids: $("#input_remove_all").val()
        }, function(data, status) {
            if ("success" == data) {
                alert("删除成功！")
            } else {
